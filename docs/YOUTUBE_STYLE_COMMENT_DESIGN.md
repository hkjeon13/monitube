...
- 정렬: 영상 댓글을 `최신순`, `오래된 순`, `추천순`으로 조회한다. 추천순은 저장된 좋아요 수를 우선하며 동률이면 최신 댓글을 먼저 표시한다.
...
- 정렬을 변경하면 진행 중인 요청을 무효화하고 목록·cursor·오류 상태를 초기화한 뒤 첫 페이지를 다시 요청한다.
- cursor는 요청한 정렬 기준을 포함하며, 다른 정렬에서 발급한 cursor를 재사용하면 서버가 거부한다.
...
```text
GET /v1/videos/{videoId}/comment-threads?sort=newest|oldest|recommended&cursor=&limit=20
...
- `newest`는 게시 시각 내림차순, `oldest`는 게시 시각 오름차순, `recommended`는 좋아요 수·게시 시각 내림차순으로 정렬한다. 모든 정렬은 댓글 ID를 최종 동률 해소 키로 사용한다.
