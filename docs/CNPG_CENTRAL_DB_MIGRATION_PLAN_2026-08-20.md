...
| Writer fence 준비 | 완료 (미활성) | API의 `MONITUBE_MAINTENANCE_READ_ONLY=true`가 모든 mutation을 `503 maintenance_read_only`로 막는 code/chart switch를 추가했다. API unit test 26개와 Helm render를 통과했고, cutover용 이미지 `cnpg-fence-6cc7b2c` (digest `sha256:12ab2dd91991af6e06a984fc619870a2e4090eb53c67195c6ae342ef4c408509)를 registry에 게시했다. 현재 release에는 아직 적용하지 않았다. |
