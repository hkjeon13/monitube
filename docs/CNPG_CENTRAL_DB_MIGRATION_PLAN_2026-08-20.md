...
| Logical replication rehearsal | 재시도 진행 중 | 첫 격리 database `monitube_logical_rehearsal_20260820_064700`는 `nlp_documents` initial copy 완료 직후 source 기본 `wal_sender_timeout=1min`으로 apply walsender가 끊겨 subscription이 `disable_on_error`로 중지됐다. production target/application endpoint에는 write가 없었고, evidence를 수집한 뒤 target database·subscription, source publication·temporary role·main/table-sync slot을 모두 정리해 잔여 0을 확인했다. 이어진 두 실행은 안전 범위에서 중단·정리했다. 두 번째는 10분 timeout 가드를 2시간으로 확장하기 위한 설정 변경 전, 세 번째는 `comments` 전체 COPY 예상 시간이 약 110분으로 2시간 timeout 여유가 작음을 확인한 뒤 4시간 가드로 재시작하기 전이었다. 현재 `monitube_logical_rehearsal_20260820_080000`이 4시간 guard로 initial copy 중이다. initial COPY가 source WAL을 보존하므로 64GiB source-free 하한 safeguard도 명시적 opt-in으로 준비했다. 성공은 모든 relation Ready, subscription enabled, slot lag gate, temporary table-sync slot 0, bounded parity 증적까지 충족한 경우에만 기록한다. |
