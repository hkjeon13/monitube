...
| Legacy-safe chart path | 완료 | 기존 Helm release가 보존한 `database.useCnpg` values와 새 chart의 `database.mode`가 충돌하던 upgrade 문제를 수정했다. 동일 values의 `helm upgrade --dry-run` 및 `kubectl diff`는 object 변경 0건이었고, chart `0.2.4`를 release revision 9로 반영한 뒤 모든 Pod restart 0·Ready를 확인했다. 이후 격리 logical rehearsal 전용으로만 release revision 12에 `wal_level=logical`, replication slot/sender 10, `wal_sender_timeout=2h`를 적용했다. 이 설정은 final cutover/soak 종료 뒤 평시 legacy mode로 되돌릴 대상이다. |
| Logical replication rehearsal | 재시도 중 | 첫 격리 database `monitube_logical_rehearsal_20260820_064700`는 `nlp_documents` initial copy 완료 직후 source 기본 `wal_sender_timeout=1min`으로 apply walsender가 끊겨 subscription이 `disable_on_error`로 중지됐다. production target/application endpoint에는 write가 없었고, evidence를 수집한 뒤 target database·subscription, source publication·temporary role·main/table-sync slot을 모두 정리해 잔여 0을 확인했다. source timeout을 2시간으로 올린 뒤 두 번째 초기 재시도도 설정 변경 전에 중단·정리했으며, 현재 `monitube_logical_rehearsal_20260820_073100`에서 동일한 격리 검증을 수행 중이다. 성공은 모든 relation Ready, subscription enabled, slot lag gate, temporary table-sync slot 0, bounded parity 증적까지 충족한 경우에만 기록한다. |
