...
| Physical restore drill | 완료 (timestamp PITR 별도) | `central-pg-data-restore-20260820-063400`이 completed Barman backup `20260819T180000`과 `targetImmediate=true`로 독립 1-instance Cluster에 복구됐다. 생성 `06:34:56Z`부터 healthy/read 검증 `06:42:48Z`까지 관측 RTO는 **472초**였고, Ready 1/1, restart 0, `pg_is_in_recovery=false`, PostgreSQL 17.11 및 비식별 central database inventory를 확인했다. 첫 실행은 target 없이 active archive tail을 따라갔고, 두 번째·세 번째 time target은 CNPG `1.27.0`이 입력 offset과 무관하게 trailing `Z`를 PostgreSQL 17 설정에 넣어 기동 전에 거부됐다. 각 실패 PV는 `Retain`으로 보존했다. timestamp PITR은 CNPG upgrade 후 별도 compatibility rehearsal로 분리하며, 현재 완료는 명시된 base backup의 physical restore/RTO 증명 범위다. |
| Legacy-safe chart path | 완료 | 기존 Helm release가 보존한 `database.useCnpg` values와 새 chart의 `database.mode`가 충돌하던 upgrade 문제를 수정했다. 동일 values의 `helm upgrade --dry-run` 및 `kubectl diff`는 object 변경 0건이었고, chart `0.2.4`를 release revision 9로 반영한 뒤 모든 Pod restart 0·Ready를 확인했다. 이후 격리 logical rehearsal 전용으로만 `wal_level=logical`, replication slot/sender 10, `wal_sender_timeout=4h`를 적용한다. 이 설정은 final cutover/soak 종료 뒤 평시 legacy mode로 되돌릴 대상이다. |
| Logical replication rehearsal | 안전 중단·미통과 (cutover 경로 미채택) | 첫 격리 database `monitube_logical_rehearsal_20260820_064700`는 `nlp_documents` initial copy 완료 직후 source 기본 `wal_sender_timeout=1min`으로 apply walsender가 끊겨 subscription이 `disable_on_error`로 중지됐다. production target/application endpoint에는 write가 없었고, evidence를 수집한 뒤 target database·subscription, source publication·temporary role·main/table-sync slot을 모두 정리해 잔여 0을 확인했다. 이어진 두 실행은 안전 범위에서 중단·정리했다. 마지막 4시간 guard 실행 `monitube_logical_rehearsal_20260820_080000`은 52분에 48 relation 중 19 Ready·29 pending인 상태에서 active writer가 temporary sync slot에 약 13.25GiB WAL을 보존하고 source free를 약 112~115GiB까지 빠르게 낮추는 것을 확인했다. 96GiB safety gate 전에 증적을 보존하고 격리 target DB·subscription, source publication·temporary role·main/table-sync slot을 모두 제거했다(잔여 slot/role 0, target production `monitube` public table 0). 따라서 active-writer logical replication은 이 source volume에서 cutover 경로로 사용하지 않는다. 계획된 writer fence 후 final logical dump/restore를 production cutover의 단일 데이터 이동 경로로 사용하며, 향후 logical rehearsal 재시도는 quiet window 또는 충분한 source WAL capacity가 별도로 확보된 경우에만 한다. |
| Rollback release package | 완료 (pre-cutover) | source release revision 13의 Helm values, rendered manifest, status를 `/data/psyche/backups/monitube/cnpg-cutover-prep-20260820T080700Z`에 mode 0600으로 보관하고 SHA-256을 기록했다. 이 파일은 Secret 값을 포함하지 않으며 target 첫 write 전 legacy endpoint로 복귀할 때의 정확한 release 기준이다. |
