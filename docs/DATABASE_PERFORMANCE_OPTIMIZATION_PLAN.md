...
2026-07-18 기준으로 이 문서의 애플리케이션·schema·운영 자동화 구현은 완료했으며, 운영 데이터 cutover는 아직 실행하지 않았다.
...
- API/worker 전체 테스트 70개 통과
- web TypeScript 검사와 production build 통과
- Python bytecode, Bash/sh 문법, Compose config, `git diff --check` 통과
- 원격 PostgreSQL 16 disposable DB에서 migration 001~015 전체 적용 및 `--check` 통과
- 원격 기존 데이터의 active analysis/result 중복 제약 충돌 사전 점검 통과
- 원격 Redis keyspace가 비어 있고 현재 AOF 데이터가 0임을 확인

### 14.3 운영 반영 전 남은 gate

1. 원격 `.env`의 PostgreSQL·MinIO 예제 credential을 회전한다.
2. 검증된 backup을 생성한 뒤 migration 015와 runtime 설정을 배포한다.
3. write/API/search의 안전 flag만 먼저 켜고 rollup read·Explore rollup·Redis cache는 끈 상태로 확인한다.
4. rollup backfill과 reconciliation을 실행해 `ready`, missing 0, mismatch 0을 확인한다.
5. 고정 workload의 p50/p95/p99, pool wait, temp/WAL, RSS, queue age를 측정한 뒤 read/cache flag를 단계적으로 켠다.
6. off-host backup과 runtime/migration DB role 분리는 후속 운영 hardening으로 완료한다.

현재 배포 script는 예제 PostgreSQL 또는 MinIO credential을 감지하면 service나 DB를 변경하기 전에 실패하도록 구성했다. credential 회전은 실행 중인 DB·object storage 접근 정보를 바꾸는 운영 작업이므로 별도 승인과 maintenance 시점 확정 후 진행한다.
