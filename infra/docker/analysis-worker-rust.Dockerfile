...
RUN cargo build --locked --release --package monitube-analysis-worker-rust
...
COPY --from=builder /workspace/target/release/monitube-analysis-worker-rust /usr/local/bin/monitube-analysis-worker-rust

USER nobody
ENTRYPOINT ["/usr/local/bin/monitube-analysis-worker-rust"]
