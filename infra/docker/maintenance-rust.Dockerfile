...
RUN cargo build --locked --release --package monitube-maintenance-rust
...
COPY --from=builder /workspace/target/release/monitube-maintenance-rust /usr/local/bin/monitube-maintenance-rust

USER nobody
ENTRYPOINT ["/usr/local/bin/monitube-maintenance-rust"]
