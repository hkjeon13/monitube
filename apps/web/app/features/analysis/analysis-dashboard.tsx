    ...
    <section className="analysis-page" id="analysis" aria-labelledby="analysis-title">
      ...
      {data && <>
        ...
        <div className="analysis-layout">
          ...
          {view !== "videos" && <section className="analysis-panel analysis-panel-words"><div className="analysis-panel-heading"><div><p className="section-kicker">LANGUAGE</p><h2>자주 등장한 명사·동사</h2></div><span>{formatCount(sampled)} / {formatCount(total)} 표본</span></div>{data.topWords.length ? <ol className="analysis-word-list">{data.topWords.map((word, index) => <li key={word.label}><span>{index + 1}</span><strong>{word.label}</strong><small>{formatCount(word.count)}</small></li>)}</ol> : <p className="analysis-empty">분석할 명사·동사가 없습니다.</p>}</section>}
        </div>
