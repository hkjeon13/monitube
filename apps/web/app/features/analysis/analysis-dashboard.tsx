    ...
    <section className="analysis-page" id="analysis" aria-labelledby="analysis-title">
      ...
      {data && <>
        ...
        <div className="analysis-layout">
          ...
          {view !== "comments" && <section className="analysis-panel analysis-panel-words"><div className="analysis-panel-heading"><div><p className="section-kicker">VIDEO TF-IDF</p><h2>영상 대본 핵심어</h2></div><span>{formatCount(data.keywordCoverage.indexedVideoDocuments)}개 대본</span></div>{data.videoKeywords.length ? <ol className="analysis-word-list">{data.videoKeywords.map((keyword, index) => <li key={keyword.term}><span>{index + 1}</span><strong>{keyword.term}</strong><small>점수 {keyword.score.toFixed(2)} · {formatCount(keyword.documentCount)}개 영상</small></li>)}</ol> : <p className="analysis-empty">인덱싱된 영상 대본 핵심어가 없습니다.</p>}</section>}

          {view !== "videos" && <section className="analysis-panel analysis-panel-words"><div className="analysis-panel-heading"><div><p className="section-kicker">COMMENT TF-IDF</p><h2>댓글 핵심어</h2></div><span>{formatCount(data.keywordCoverage.indexedCommentDocuments)}개 댓글</span></div>{data.commentKeywords.length ? <ol className="analysis-word-list">{data.commentKeywords.map((keyword, index) => <li key={keyword.term}><span>{index + 1}</span><strong>{keyword.term}</strong><small>점수 {keyword.score.toFixed(2)} · {formatCount(keyword.documentCount)}개 댓글</small></li>)}</ol> : <p className="analysis-empty">인덱싱된 댓글 핵심어가 없습니다.</p>}</section>}
        </div>
