  ...
  return (
    <section className="explore-section" id="explore" aria-label="Explore" tabIndex={-1}>
      ...
      <form className="explore-search" role="search" onSubmit={onSubmit}>
        ...
        <input id="collected-search" value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder={scope === "videos" ? "영상 제목, 채널, 대본 검색" : scope === "comments" ? "댓글 검색" : "영상 제목, 채널, 대본, 댓글 검색"} type="search" autoComplete="off" />
      ...
      {loading && data.channels.length === 0 ? <p className="explore-loading">수집 라이브러리를 불러오는 중입니다…</p> : hasSearchQuery ? (
        <section className="collected-search-results" aria-live="polite" aria-label="통합 검색 결과">
          {searchLoading && !searchResults ? (
            ...
            <div className="search-result-columns">
              {submittedScope !== "comments" && <section><h4>동영상</h4>{searchResults?.videos.map((result) => <button className="search-video-result" type="button" key={result.video.id} onClick={(event) => onOpenVideo(result.video, event.currentTarget)}><img src={youtubeThumbnail(result.video.youtubeVideoId)} alt="" /><span><strong>{result.video.title}</strong>{result.transcriptSnippet && <q>{result.transcriptSnippet.text}</q>}<small>{result.matchedFields.map(searchFieldLabel).join(" · ")}{result.transcriptSnippet ? ` · ${Math.floor(result.transcriptSnippet.startMs / 1000)}초` : ` · 유사도 ${Math.round(result.score * 100)}%`}</small></span><ChevronRightIcon aria-hidden="true" /></button>)}{!searchLoading && (searchResults?.videos.length ?? 0) === 0 && <p className="search-empty">일치하는 동영상이 없습니다.</p>}</section>}
