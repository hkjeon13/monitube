...
class SourceService(ApplicationService):
    ...
    def refresh_source(
        ...
        *,
        ...
        """Queue one immediate refresh while retaining the automatic cadence."""

        existing = self.repository.get_source(source_id, owner_id=owner_id)
        if existing.type is SourceType.VIDEO:
            raise ValueError("Manual refresh is available for channel and keyword sources")
        config = self._canonical_config(existing.type, existing.config)
        canonical_key, aliases = self._canonical_target(existing.type, config)
        submission = self.repository.submit_collection_request(
        ...
        if submission.disposition in {"queued", "successor_queued"}:
            self.repository.mark_target_manual_dispatch(
                target_id=submission.target.id,
                dispatched_at=submission.request.created_at,
            )
        return self._submission_contract(submission)
    ...
    def get_video_transcript(
        self, youtube_video_id: str, *, owner_id: str
    ) -> VideoTranscriptResponse:
        transcript = self.repository.get_video_transcript(
            youtube_video_id, owner_id=owner_id
        )
        return VideoTranscriptResponse(
            videoId=transcript.youtube_video_id,
            provider=transcript.provider,
            requestedLanguage=transcript.requested_language,
            resolvedLanguage=transcript.resolved_language,
            languageName=transcript.language_name,
            selectionReason=transcript.selection_reason,
            transcriptType=transcript.transcript_type,
            isAutoGenerated=transcript.is_auto_generated,
            isTranslated=transcript.is_translated,
            state=transcript.state,
            fullText=transcript.full_text,
            fetchedAt=transcript.fetched_at,
            lastAttemptedAt=transcript.last_attempted_at,
            errorCode=transcript.error_code,
            segments=[
                TranscriptSegment(
